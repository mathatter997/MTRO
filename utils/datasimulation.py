...
class DataSimulation(object):
    ...
    def run(self, ranker_tuples):
        ...
        assert all(output.finished() for output in
                   self.run_outputs.values()), 'Program exiting but not all outputs were finished.'
    ...
    def wait_for_output(self, timeout=30):  # 0):
        ...
        try:
            while True:
                ...
                print('Output %d: %s on dataset %s. (%d/%d)' % (
                self._outputs_found, output_key[0], output_key[1], sim_output.run_index + 1,
